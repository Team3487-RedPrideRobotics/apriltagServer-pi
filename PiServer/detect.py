...
detectorOptions = json.load(open("detector.json"))


detector = Detector(
	families=detectorOptions["families"],
	nthreads=detectorOptions["nthreads"],
	quad_decimate=detectorOptions["quad_decimate"],
	quad_sigma=detectorOptions["quad_blur"],
	refine_edges=detectorOptions["refine_edges"],
	decode_sharpening=detectorOptions["decode_sharpening"],
	debug=detectorOptions["debug"]
)
...
def get_pose(frame):
		...
		tags = detector.detect(grey, estimate_tag_pose=True, camera_params=camera_matrix, tag_size=tag_size)
		robot_pose = None
		if(not tags):
			return(frame,None,None,None)
		for tag in tags:
			pose = np.concatenate((np.concatenate((tag.pose_R,tag.pose_t),axis=1),np.array([[0,0,0,1]])),axis=0)
			if(tag.tag_id != 0):
				return(frame,None,None,None)
			...
			return frame,asin(robot_pose[2][0]),robot_pose[0][3],robot_pose[2][3]
